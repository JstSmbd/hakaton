...
def holiday(res, req, ses):
    ...
    if (check_tokens(["расскажи", "скажи"], req) and check_tokens(["про", "о"], req)) or \
    ...
    elif check_tokens(["хватит", "достаточно"], req):
        ses["state"] = None
        ...
        if holidays:
            res["response"]["text"] = f"{'; '.join(holidays)}! Вам рассказать что-то про какой-нибудь праздник или может сказать " \
                              "какой праздник в другую дату?"
        else:
